...
dwrModule.provider("$dwr", function () {
    var provider = this;
    /**
     * @param webRoot 设置web服务器跟路径。
     */
    provider.setWebRootPath = function (webRoot) {
        dwrConf._contextPath = webRoot;
    };

    /**
     *
     * @returns [String] 文本根路径
     */
    provider.getWebRootPath = function () {
        return dwrConf._contextPath;
    };

    /**
     *
     * @param dwrRootPath 设置dwrservlet的web路径， 如果不设置默认为 web根路径加/dwr
     */
     ...
     * @returns [String] dwrservlet路径。
     */
    provider.getDwrServletPath = function () {
        if (dwrConf._pathToDwrServlet != null) {
            return dwrRootPath;
        } else {
    ...
    dwrConf._contextPath = "http://localhost:8080/icbc/mo/emerchant";
    /** The default path to the DWR servlet */
    dwrConf._pathToDwrServlet = null;
    /** Is GET enabled? */
    dwrConf._allowGetButMakeForgeryEasier = "true";
    /** The script prefix to strip in the case of scriptTagProtection. */
    dwrConf._scriptTagProtection = "throw 'allowScriptTagRemoting is false.';";
    /** Custom path to the DWR servlet */
    dwrConf._overridePath = "";
    /** Custom contextPath */
    dwrConf._overrideContextPath = "";
    /** Extra attributes to append to the DWRSESSIONID cookie (domain, secure, etc) */
    dwrConf._cookieAttributes = "";
    /** Do we use XHR for reverse ajax because we are not streaming? */
    dwrConf._useStreamingPoll = "true";
    /** These URLs can be configured from the server */
    dwrConf._ModePlainCall = "/call/plaincall/";
    dwrConf._ModePlainPoll = "/call/plainpoll/";
    dwrConf._ModeHtmlCall = "/call/htmlcall/";
    dwrConf._ModeHtmlPoll = "/call/htmlpoll/";
    ...
    this.$get = ["$q", '$log','$state','$cookies','$ionicLoading', function ($q, $log,$state,$cookies,$ionicLoading) {
        ...
        (function () {
             ...
             * @param {boolean} activeReverseAjax true to enable low latency reverse ajax
             */
            dwr.engine.setActiveReverseAjax = function (activeReverseAjax) {
                if (activeReverseAjax) {
                    ...
                    if (!dwr.engine._retryIntervals || dwr.engine._retryIntervals.length === 0) {
            ...
            dwr.engine.defaultErrorHandler = function (message, ex) {
                ...
                if (dwrConf.isDebug == "true") alert(message);
            };
            ...
            /** Is GET enabled? */
            dwr.engine._allowGetButMakeForgeryEasier = dwrConf._allowGetButMakeForgeryEasier;

            /** The script prefix to strip in the case of scriptTagProtection. */
            dwr.engine._scriptTagProtection = dwrConf._scriptTagProtection;

            /** The default path to the DWR servlet */
            dwr.engine._pathToDwrServlet = provider.getDwrServletPath();

            /** Custom path to the DWR servlet */
            dwr.engine._overridePath = dwrConf._overridePath;
            ...
            /** The webapp's contextPath */
            dwr.engine._contextPath = provider.getWebRootPath();

            /** Custom contextPath */
            dwr.engine._overrideContextPath = dwrConf._overrideContextPath;
            ...
            /** Do we use XHR for reverse ajax because we are not streaming? */
            dwr.engine._useStreamingPoll = dwrConf._useStreamingPoll;

            dwr.engine._pollOnline = true;

            /** These URLs can be configured from the server */
            dwr.engine._ModePlainCall = dwrConf._ModePlainCall;
            dwr.engine._ModePlainPoll = dwrConf._ModePlainPoll;
            dwr.engine._ModeHtmlCall = dwrConf._ModeHtmlCall;
            dwr.engine._ModeHtmlPoll = dwrConf._ModeHtmlPoll;

            /** Do we make the calls async? Default to 'true' */
            dwr.engine._async = dwrConf._async;
            ...
            dwr.engine._unloader = function () {
                ...
                if (dwr.engine._isNotifyServerOnPageUnload) {
                    ...
                    batch = {
                        ...
                        handlers: [
                            { exceptionHandler: null, callback: null }
                        ]
                    };
                ...
                convertArray: function (batch, directrefmap, otherrefmap, data, name, depth) {
                    ...
                    if (dwr.engine.util.ieCondition("if lte IE 7")) {
                        ...
                        for (i = 0; i < data.length; i++) {
                            if (i !== 0) buf.push(",");
                            batch.paramCount++;
                            childName = "c" + batch.map.callCount + "-e" + batch.paramCount;
                ...
                convertObject: function (batch, directrefmap, otherrefmap, data, name, depth) {
                    ...
                    for (element in elementset) {
                        if (typeof data[element] != "function" && !dwr.engine._excludeObjectAttributes[element]) {
                            batch.paramCount++;
                            var childName = "c" + batch.map.callCount + "-e" + batch.paramCount;
                ...
                send: function (batch) {
                    ...
                    if (!dwr.engine._dwrSessionId) {
                        ...
                        var idbatch = {
                            ...
                            headers: batch.headers, preHooks: [],
                            postHooks: [function () {
                                dwr.engine._internalOrdered = false;
                            }],
                            timeout: dwr.engine._timeout,
                            errorHandler: batch.errorHandler,
                            globalErrorHandler: batch.globalErrorHandler,
                            warningHandler: batch.warningHandler,
                            textHtmlHandler: batch.textHtmlHandler,
                            globalTextHtmlHandler: batch.globalTextHtmlHandler,
                            path: batch.path,
                            handlers: [
                                {
                        ...
                        if (batch.httpMethod === "GET") {
                            ...
                            if (batch.headers != null) {
                                for (var prop in batch.headers) {
                                    ...
                                    formInput.setAttribute("name", "a-" + prop);
